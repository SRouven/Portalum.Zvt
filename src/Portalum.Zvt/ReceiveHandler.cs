        ...
        public virtual ProcessData ProcessData(Span<byte> data)
        {
            ...
            if (this._missingDataOfExpectedPackage == 0)
            {
                ...
                if (data.Length > apduInfo.PackageSize)
                {
                    ...
                    this.AbortReceived?.Invoke("Apdu data part corrupt"); //SS 15.11.2022: Erg�nzung zu Portalum um Abbruch Fehlerhaften Apdu Daten zu erreichen. Dies hatte am A77 Fehler behoben. Ich bin aber nicht ganz sicher, ob das eingebaute Verhalten immer korrekt ist.                    
                    return new ProcessData{ State = ProcessDataState.CannotProcess };
                }
